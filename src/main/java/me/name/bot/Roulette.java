...
public class Roulette {
	public static void roulette(Server server, boolean routine) {
		if (!routine) {
			if (Math.random() > 0.5) return;
			exec(null, server.guild, false);
			return;
		}
		if (Math.random() > server.chances) return;
		exec(server, server.guild, true);
	}

	private static void exec(Server server, @NotNull Guild guild, boolean routine) {
